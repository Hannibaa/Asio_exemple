...
public:
	...
	void start_server(uint16_t port) {
		...
		acceptor_.async_accept(handler->socket(), [=](auto ecc) {
			handle_new_connection(handler, ecc);
			});

		// start thread pool to process the asio events
...
private:
	...
	void packet_send_done(std::error_code const& ec) {
		if (!ec) {
